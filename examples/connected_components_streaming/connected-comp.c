						    ...
						    int64_t * restrict tree)
{
  ...
  OMP("omp parallel") {
    ...
    while(1) {
      OMP("omp single")
	change = 0;
      OMP("omp for reduction(+:change)")
	for(uint64_t l = 0; l < S->ETA[0].high; l++) {
	  struct stinger_eb * current = ebpool + S->ETA[0].blocks[l];
	  uint64_t u = current->vertexID;
	  for(uint64_t j = 0; j < stinger_eb_high(current); j++) {
	    if (stinger_eb_is_blank(current, j)) continue;
	    uint64_t v = current->edges[j].neighbor;
	    if(d[u] < d[v]) {
				   ...
				   int64_t * restrict T)
{
  ...
  struct stinger_eb * start = ebpool + S->ETA[0].blocks[0];
  uint64_t stop = S->ETA[0].high;
  ...
  OMP("omp parallel") {
	...
	change = 0;
      OMP("omp for reduction(+:change)")
	for(uint64_t l = 0; l < stop; ++l) {
	  struct stinger_eb * current = start + l;
	  uint64_t u = current->vertexID;
	  for(uint64_t j = 0; j < current->high; ++j) {
	    uint64_t v = current->edges[j].neighbor;
	    if(d[u] < d[v]) {
	      d[d[v]] = d[u];
	      ++change;
